...
float AFPSCharacter::TakeDamage(float Damage, FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
	...
	if (CauserPawn)
	{
		...
		if (Health <= 0)
		{
			...
			if (OtherPS)
			{
			OtherPS->SetScore(OtherPS->GetScore() + 1);
			}
...
void AFPSCharacter::ClientRagDoll_Implementation(FVector ImpactDirection)
{
	...
	FP_Mesh->AddImpulseAtLocation(ImpactDirection * 30000.f, GetActorLocation());
